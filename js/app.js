...
class App {
  constructor() {
    ...
    this.privateLog = [];
    this.curFx = FX[0];
    this.serverMenu = null;      // {title, items, tabs} from server [4/3]
  ...
  async loadSplashBg() {
    ...
    for (const n of ["tile_splash_bg", "tile_splash_bg_night", "tile_splash_bg_shade"]) {
      const img = await loadImage(`graphics/${n}.png`).catch(() => null);
      this.splashTiles.push(img);
    }
    try { $("#splash-logo").src = ASSETS + "graphics/title_logo.png"; } catch (e) {}
    try { $("#splash-spinner").src = ASSETS + "graphics/load_spinner.png"; } catch (e) {}
    this.splashRaf();
    this.resizeSplash();
  ...
  renderLobby() {
    ...
    if (this.serverMenu && this.serverMenu.items) {
      this.ui.renderServerMenu(this.serverMenu, (item, i) => this.onServerPick(item, i));
      return;
    }
    this.ui.renderLobby(LOCATIONS, () => Math.floor(50 + Math.random() * 400), (loc) => this.enterRoom(loc));
  }

  async onServerPick(item, i) {
    const m = this.serverMenu;
    if (m.tabs && item.key === "__tab__") {
      this.net.selectTab(item.tab_no);
      return;
    }
    if (m.tabs) {
      this.net.selectCityItem(i, item.key);
      return;
    }
    this.net.selectMenuItem(i, item.key);
  }
  ...
  onNet(ev) {
    ...
    switch (ev.type) {
      ...
      case "user_enter": {
        const u = ev.user;
        if (!this.renderer) {
          if (this.onlineRoom && this.chat) this.chat.add({ type: "sys", text: "Вошёл: [b]" + u.nick + "[/b]" });
          break;
        }
      ...
      case "menu": {
        this.serverMenu = { title: ev.title, items: ev.items };
        this.setScreen("lobby");
        break;
      }
      case "city": {
        const tabs = (ev.tabs || []).map((t) => ({ name: t.name, key: "__tab__", tab_no: t.no }));
        const items = tabs.concat(ev.items);
        this.serverMenu = { title: ev.title, items, tab_cur: ev.tab_cur, tabs: true };
        this.setScreen("lobby");
        break;
      }
      case "dialog": {
        this.serverDialog = ev;
        this.showServerDialog(ev);
        break;
      }
      case "start": {
        this.toast("Новый аккаунт создан — продолжаем...");
        break;
      }
      case "room_open": {
        this.enterServerRoom(ev);
        break;
      }
    }
  }
  ...
  showServerDialog(d) {
    ...
    $("#menu-panel .panel-head span").textContent = d.title || "Сообщение";
    body.textContent = "";
    ...
    for (const a of d.answers) {
      ...
      btn.addEventListener("click", () => {
        this.ui.hideMenu();
        this.net.answerDialog(a.key);
      });
      body.appendChild(btn);
    }
  }

  async enterServerRoom(ev) {
    this.onlineRoom = true;
    this.roomId = String(ev.loc_id);
    $("#room-title").textContent = ev.title || objectToTitle(ev.loc_id);
    this.setScreen("room");
    this.chat = new Chat($("#chat-log"), {
      onNickTap: (e) => this.toast(e.nick || ""),
      onRemove: (e) => { e && this.chat.add({ type: "sys", text: "(удалено)" }); },
    ...
    this.chat.setSmiles(this.smiles);
    if (this.renderer) { this.renderer = null; $("#room-canvas").getContext("2d").clearRect(0, 0, 1, 1); }
...
function objectToTitle(v) {
  if (v == null) return "Комната";
  return "Комната #" + v;
}
